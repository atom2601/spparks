...
\endverbatim
 *
 * \param  spk             Pointer to SPPARKS instance
...
PotentialFileReader::PotentialFileReader(SPPARKS *spk, const std::string &filename,
    ...
    unit_convert(auto_convert)
{
    ...
    try {
        ...
        if (!reader) {
            std::string formattedMesg = fmt::format("cannot open {} potential file {}: {}", 
...
PotentialFileReader::PotentialFileReader(SPPARKS *spk, const std::string &filename,
    ...
    PotentialFileReader(spk, filename, potential_name, " potential", auto_convert)
{
}
...
/* Read a line but ifnore its content */
