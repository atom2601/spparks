        ...
        double **x, int *type)
{
    ...
    // spparks will not be doing calcualtions with per-atom energies therefore commented out
    ...
    for (jj = 0; jj < jnum; jj++) {
        ...
        if (rsq < cutforcesq) {
            ...
            // rhoip = derivative of (desnity at atom j due to atom i)
            // rhojp = derivative of (density of atom i due to atom j)
...
void PairEAM::allocate()
{
    ...
    memory->create(cutsq,n+1,n+1,"pair:cut");
...
void PairEAM::coeff(int narg, char **arg)
{
    ...
    // read funcflk file if hasn't already been read
    ...
    for (int i = ilo; i <= ihi; i++) {
      for (int j=MAX(jlo,i); j <= jhi; j++) {
        if (i ==j) {
            setflag[i][i] = 1;
            map[i] = ifuncfl;
            // atom->set_mass(FLERR,i,funcfl[ifuncfl].mass); // mass in not a paramater of interest in spparks
            count++;
        }
        scale[i][j] = 1.0;
      }
...
void PairEAM::init_style()
{
    ...
    // neighbor->add_request(this); dont think this is needed
    // embedstep = -1; taken from lammmps, not used here
}
...
double PairEAM::init_one(int i, int j)
{
    ...
    // for funcfl could be ultiple files
...
void PairEAM::file2array()
{
    ...
    // type2frho[i] = which frho array (0 to nfrhro-1) each atom type maps to
    // if atom types doesn't point to file (non-EAM atom in pair hybrid)
    ...
    // type2rho[i][j] = which rhor array (0 to nrhor-1) each type pair maps to
    ...
    for (i = 0; i < nfuncfl; i++) {
      ...
      for (j = 0; j <= i; j++) {
        ...
        for (m = 1; m <= nr; m++) {
            ...
            zri = cof1*ifile->zr[k-1] + cof2*ifile->zr[k] + 
              cof3*ifile->zr[k+1] + cof4*ifile->zr[k+1];
            ...
            zrj = cof1*jfile->zr[k-1] + cof2*jfile->zr[k] + 
              cof3*jfile->zr[k-1] + cof4*jfile->zr[k+1];
      ...
      // swap indicies when irow < icol to stay lower triangular
      // if map = -1 (non-EAM atom in pair hybrid):
      //    type 2z2r is not used by non-opt
      //    but set type 2z2r to 0 since accessed by opt
                       ...
                       double &fforce)
{
    ...
    // if ((comm->get_me() == 0) && (embedstep != update->ntimestep)) { // taken from lammps, not used here
