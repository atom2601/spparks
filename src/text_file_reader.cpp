...
/* ----------------------------------------------------------------------
   ...
   Code based off that text_file_reader.cpp file in LAMMPS made by
...
{
    // concatenate lines until have nparams word
    int n = 0;
    int nwords = 0;
    ...
    while (nwords == 0 || nwords < nparams) {
        ...
        if (ptr == nullptr) {
            ...
            if (nwords > 0 && nwords < nparams) {
                throw EOFException(fmt::format("Incorrect format in {} file! {}/{} paramaters", filetype,
                                    nwords, nparams));
            }
            return nullptr;
        }
